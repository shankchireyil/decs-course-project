...
// Thread-local latency buffer to reduce mutex contention
#define LATENCY_BUFFER_SIZE 100  // Reduced size to prevent stack overflow
typedef struct {
    long latencies[LATENCY_BUFFER_SIZE];
    int count;
} LatencyBuffer;

// Response data structure for cURL - reusable buffer
struct ResponseData {
    char *data;
    size_t size;
    size_t capacity;
...
typedef struct 
{
    ...
    CURL *curl_handle;  // cURL handle for HTTP requests
    struct ResponseData response_buffer;  // Reusable response buffer
    LatencyBuffer latency_buffer;  // Thread-local latency buffer
    struct curl_slist *headers;  // HTTP headers for keep-alive
} ThreadStats;
...
// Optimized callback to write response data - reuses buffer
static size_t WriteCallback(void *contents, size_t size, size_t nmemb, struct ResponseData *response)
{
    size_t total_size = size * nmemb;
    size_t new_size = response->size + total_size + 1;
    
    // Only reallocate if needed
    if (new_size > response->capacity) {
        size_t new_capacity = new_size * 2; // Double capacity to reduce reallocations
...
// Reset response buffer for reuse
static void reset_response_buffer(struct ResponseData *response) {
    response->size = 0;
...
// Initialize cURL handle for HTTP requests with optimizations
CURL* init_curl_handle(struct ResponseData *response_buffer) 
{
    CURL *curl = curl_easy_init();
    if (!curl) return NULL;
    
    // Initialize response buffer
    response_buffer->data = malloc(4096);  // Start with 4KB buffer
    ...
    // Set optimized options for high-performance load testing
    curl_easy_setopt(curl, CURLOPT_TIMEOUT, 10L);
    curl_easy_setopt(curl, CURLOPT_CONNECTTIMEOUT, 2L);  // Faster connection timeout
    ...
    curl_easy_setopt(curl, CURLOPT_TCP_KEEPIDLE, 30L);    // Shorter keep-alive
    curl_easy_setopt(curl, CURLOPT_TCP_KEEPINTVL, 15L);   // Shorter intervals
    curl_easy_setopt(curl, CURLOPT_MAXCONNECTS, 10L);     // Connection pooling
    ...
    curl_easy_setopt(curl, CURLOPT_NOPROGRESS, 1L);       // No progress callback
    
    // HTTP version and connection optimizations
    ...
    // DNS optimizations
    curl_easy_setopt(curl, CURLOPT_DNS_CACHE_TIMEOUT, 300L);  // Cache DNS for 5 minutes
    
    return curl;
}

// Optimized HTTP request using reusable buffer
int send_http_request(CURL *curl, struct ResponseData *response_buffer, LatencyBuffer *latency_buffer,
                     const char* method, int key, const char* value, long* latency) 
{
    if (!curl) return 0;
    
    char url[256];
    snprintf(url, sizeof(url), "http://%s:%d/%d", SERVER_IP, SERVER_PORT, key);
    
    // Reset response buffer for reuse
    reset_response_buffer(response_buffer);
    ...
    if (strcmp(method, "GET") == 0) 
    {
        ...
        curl_easy_setopt(curl, CURLOPT_CUSTOMREQUEST, NULL);  // Clear custom request
    }
    else if (strcmp(method, "POST") == 0) 
    {
        curl_easy_setopt(curl, CURLOPT_POSTFIELDS, value);
        curl_easy_setopt(curl, CURLOPT_POSTFIELDSIZE, (long)strlen(value));
        curl_easy_setopt(curl, CURLOPT_CUSTOMREQUEST, NULL);  // Clear custom request
    }
    ...
    *latency = end - start;
    add_latency_local(latency_buffer, *latency);
    ...
    if (res == CURLE_OK) 
    {
        ...
        success = (response_code == 200 || response_code == 404);  // 404 is valid for GET misses
    }
...
void* client_thread(void* arg) 
{
    ...
    // Initialize cURL handle with response buffer
    stats->curl_handle = init_curl_handle(&stats->response_buffer);
    if (!stats->curl_handle) 
    {
        printf("Thread %d: Failed to initialize cURL\n", stats->thread_id);
        return NULL;
    }
    
    // Set up HTTP headers for keep-alive per thread
    stats->headers = NULL;
    stats->headers = curl_slist_append(stats->headers, "Connection: keep-alive");
    stats->headers = curl_slist_append(stats->headers, "Keep-Alive: timeout=5, max=1000");
    curl_easy_setopt(stats->curl_handle, CURLOPT_HTTPHEADER, stats->headers);
    ...
    while (!stop_test) 
    {
        ...
        switch (stats->type) 
        {
            case PUT_ALL:
                ...
                if (rand() % 2 == 0) {
                    success = send_http_request(stats->curl_handle, &stats->response_buffer, &stats->latency_buffer, "POST", key, value, &latency);
                } else {
                    success = send_http_request(stats->curl_handle, &stats->response_buffer, &stats->latency_buffer, "DELETE", key, NULL, &latency);
                }
                break;
                
            case GET_ALL:
                key = rand() % KEY_RANGE + 1;
                success = send_http_request(stats->curl_handle, &stats->response_buffer, &stats->latency_buffer, "GET", key, NULL, &latency);
                break;
        ...
        // Reduce CPU usage by yielding occasionally
        if (request_count % 100 == 0) {
            usleep(1);  // Microsleep to reduce CPU usage
        }
    }
    ...
    // Cleanup cURL handle, headers, and response buffer
    if (stats->headers) {
        curl_slist_free_all(stats->headers);
    }
    if (stats->curl_handle) {
        curl_easy_cleanup(stats->curl_handle);
    }
    if (stats->response_buffer.data) {
        free(stats->response_buffer.data);
    }
...
void print_usage(const char* program_name) {
    ...
    printf("  %s -t 100 -o put-all               # Disk-bound workload\n", program_name);
    printf("  %s -t 20 -o mixed -r 80 -w 15      # Custom mixed workload\n", program_name);
}
...
int main(int argc, char* argv[]) 
{
        ...
        switch (opt) 
        {
            ...
            case 'o':
                ...
                else 
                {
                    ...
                    printf("Valid types: get, post, delete, mixed, put-all, get-all, get-popular, get-put\n");
                    return 1;
                }
                break;
            case 'r':
    ...
    // Validate percentages for mixed workload
    if (req_type == MIXED) 
    {
    ...
    // Allocate memory for latency tracking (increased for thread-local buffers)
    ...
    // Allocate thread arrays on heap to prevent stack overflow
    pthread_t *threads = malloc(sizeof(pthread_t) * MAX_THREADS);
    ThreadStats *stats = calloc(MAX_THREADS, sizeof(ThreadStats));
    ...
    printf("=== CivetWeb Key-Value Server Load Test ===\n");
    printf("Threads: %d\n", num_threads);
    printf("Duration: %d seconds\n", test_duration);
    printf("Server: http://%s:%d\n", SERVER_IP, SERVER_PORT);
    
    // Display workload type with description
    printf("Workload: ");
    switch (req_type) {
        case GET:
            printf("GET only (read-only)\n");
            break;
        case POST:
            printf("POST only (write-only)\n");
            break;
        case DELETE:
            printf("DELETE only (delete-only)\n");
            break;
        case MIXED:
            printf("Mixed (%.1f%% read, %.1f%% write, %.1f%% delete)\n", 
                   read_percentage, write_percentage, delete_percentage);
            break;
        case PUT_ALL:
            printf("PUT_ALL (50%% POST, 50%% DELETE - disk-bound, cache misses)\n");
            break;
        case GET_ALL:
            printf("GET_ALL (unique keys - disk-bound, cache misses)\n");
            break;
        case GET_POPULAR:
            printf("GET_POPULAR (popular keys 1-%d - CPU/memory-bound, cache hits)\n", POPULAR_KEYS);
            break;
        case GET_PUT:
            printf("GET_PUT (60%% GET, 25%% POST, 15%% DELETE - mixed performance)\n");
            break;
    }
    
    printf("Key range: 1 to %d\n", KEY_RANGE);
    printf("Connection: HTTP with cURL\n");
    printf("\nStarting load test...\n");
